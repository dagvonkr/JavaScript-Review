...
var sentence = "Dev Mountain is the best chgjfkjsnfjknfjkn";

var longest = function(str) {
	var splitToWords = str.split(' ');
	var currentLongest = splitToWords[0];
	for (var i = 1; i < splitToWords.length; i++) {
		if (currentLongest.length < splitToWords[i].length) {
			currentLongest = splitToWords[i];
...
var capitalize = function(myPoem) {
	var splitToLetters = myPoem.split('');
	// console.log(splitToLetters[0]);
	for (var i = 0; i < splitToLetters.length; i++) {
		if (splitToLetters[i] === ' ') {
			var letterToCap = splitToLetters[i + 1];
			splitToLetters.splice(i + 1, 1, letterToCap.toUpperCase());
		}	
	}	
	return splitToLetters.join('');
}
